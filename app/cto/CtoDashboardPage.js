...
import { MirthControlCard } from "../../components/cto-dashboard/MirthControlCard";
...
export default function CtoDashboardPage({
  defaultDashboardTab = "cto",
  allowCtoTab = true,
  showCtoTools = true,
  titleBadge = "CTO Control Plane",
  subtitle = null,
} = {}) {
  ...
  useEffect(() => {
    ...
    async function loadVisitMetrics() {
      setVisitMetrics((prev) => ({ ...prev, loading: true, error: "" }));
      try {
        const params = new URLSearchParams({ ts: String(Date.now()) });
        if (selectedLabId) params.set("lab_id", selectedLabId);
        ...
        if (!res.ok) throw new Error(body?.error || "Failed to load visit metrics");
        if (!cancelled) setVisitMetrics({ ...body, loading: false, error: "" });
      } catch (error) {
        if (!cancelled) {
          setVisitMetrics({ today: {}, mtd: {}, byExecutive: [], loading: false, error: error?.message || "Failed to load visit metrics" });
        }
      }
    }
    loadVisitMetrics();
