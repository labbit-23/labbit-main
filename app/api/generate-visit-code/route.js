// File: /app/api/generate-visit-code/route.js
...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY // ← Only safe (server-side)
);

export async function GET(request) {
  const { searchParams } = new URL(request.url);
  const visitDate = searchParams.get('date') || new Date();

  const dateStr = dayjs(visitDate).format('YYYYMMDD'); // e.g., 20250405

  // Get last visit code for the date (without .for('UPDATE'))
