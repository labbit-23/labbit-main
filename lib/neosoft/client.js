...
// Director-reported (2026-09-08): a report RESENT over WhatsApp from this
// app's own Admin WhatsApp report-tools flow never updated any delivery
// signal -- labit-py's automated report_sender_worker already POSTs here
// after every successful FIRST send (py_utils/workers/report_sender/
// report_sender_worker.py:1237), which labit-py's report_backend.py
// forwards, when the cutover flag is on, into labit-core's
// `labit_core.report_delivery_event` (via internal mark-requisition-
// delivered) -- the exact signal Time Motion's "Delivered" lane reads
// (result_service.py doctor_queue()'s delivered_lab_report_ids). Manual
// resends from this route only ever wrote to Supabase `report_dispatch_logs`,
// a completely separate table doctor_queue() never reads. Reusing this
// existing endpoint (rather than calling labit-core directly) means the
// same cutover-gated, Shivam-vs-labit-core routing logic already proven
// for automated sends also covers manual resends, with no new coupling.
export async function updateDeliveryStatus({ reqno, status = "S", channel = "whatsapp", message = "OK", scope = "all", testids = null }) {
  const cleanReqno = String(reqno || "").trim();
  if (!cleanReqno) return null;

  const res = await fetchWithTimeout(`${BASE_URL}/delivery/status/update`, {
    method: "POST",
    ...
    body: JSON.stringify({
      ...
      message,
      scope,
      testids: Array.isArray(testids) && testids.length > 0 ? testids : null
    })
  });

  if (!res.ok) {
    throw new Error(`NeoSoft delivery status update failed: ${res.status}`);
  }
...
export async function getOutsourcedReportMeta(reqid, testid) {
  const cleanReqid = String(reqid || "").trim();
  const cleanTestid = String(testid || "").trim();
  if (!cleanReqid || !cleanTestid) {
    throw new Error("Outsourced report meta requires reqid and testid");
  }
  ...
  const res = await fetchWithTimeout(
    `${BASE_URL}/outsourced-report/meta?${query.toString()}`,
    { cache: "no-store" }
  );

  if (!res.ok) {
    const message = await res.text().catch(() => "");
    const error = new Error(message || `NeoSoft outsourced meta failed: ${res.status}`);
...
// Generic transactional-document fetch (e-bill/bill/estimate/receipt/...) --
// same passthrough route the patient_message_jobs framework points WhatsApp's
// document.link at (labit-py /document/{kind}/{ref} -> labit-core
// DOCUMENT_KINDS registry). No staff auth needed at this hop, same as
// getReportUrl -- the labit-py route is openly fetchable (WhatsApp's own
// crawler fetches it directly for template sends); auth to labit-core
// happens server-to-server behind it.
export function getDocumentUrl(kind, ref) {
  const cleanKind = encodeURIComponent(String(kind || "").trim());
  const cleanRef = encodeURIComponent(String(ref || "").trim());
  return `${BASE_URL}/document/${cleanKind}/${cleanRef}`;
}

export function getLabReportUrl(reqid, options = {}) {
  const cleanReqid = encodeURIComponent(String(reqid || "").trim());
  const reqno = options?.reqno;
  ...
  if (testids !== undefined && testids !== null && String(testids).trim() !== "") {
    params.set("testids", String(testids).trim());
  }
  if (chkrephead !== undefined && chkrephead !== null && String(chkrephead).trim() !== "") {
    params.set("chkrephead", String(chkrephead).trim());
  }
  ...
  const query = params.toString();
  if (query) return `${BASE_URL}/lab_report/${cleanReqid}?${query}`;
  return `${BASE_URL}/lab_report/${cleanReqid}`;
}
...
export function getOutsourcedReportUrl(reqid, testid, options = {}) {
  const cleanReqid = String(reqid || "").trim();
  const cleanTestid = String(testid || "").trim();
  const params = new URLSearchParams({
    reqid: cleanReqid,
  ...
  if (options?.reqno) params.set("reqno", String(options.reqno).trim());
  if (options?.header_mode) params.set("header_mode", String(options.header_mode).trim());
  if (options?.chkrephead !== undefined && options?.chkrephead !== null) {
    params.set("chkrephead", String(options.chkrephead).trim());
  }
  return `${BASE_URL}/outsourced-report?${params.toString()}`;
}
