...
import { getLatestReportUrl, getReportStatus, getReportUrl, getTrendReportUrl, lookupReports } from "@/lib/neosoft/client";
...
function parseSlotInput(rawInput, context = {}) {
  ...
  for (const [slotId, slotLabel] of Object.entries(slotMap)) {
    const normalizedLabel = normalize(slotLabel);
    if (normalizedLabel && rawCandidates.includes(normalizedLabel)) {
      return {
        slotId,
        slotName: slotLabel
      };
  ...
  const uuidMatch = rawText.match(
    /([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})/i
  );
  if (uuidMatch) {
    const inferredSlotId = uuidMatch[1];
    if (slotMap[inferredSlotId]) {
      return {
        slotId: inferredSlotId,
        slotName: slotMap[inferredSlotId]
      };
    }
  }
...
function isSmartReportEnabled({ options = {}, botFlowConfig = {} } = {}) {
  if (typeof options?.smartReportEnabled === "boolean") return options.smartReportEnabled;
  return Boolean(botFlowConfig?.smart_report_enabled);
}
...
function isNewTrendsReportEnabled() {
  return isTruthyEnvFlag(process.env.WHATSAPP_USE_NEW_TRENDS_REPORT);
}
...
function buildSmartReportDocumentUrl({ mrno, labId, publicBaseUrl }) {
  const base = String(
    publicBaseUrl ||
...
function buildExecutiveTrendReportDocumentUrl({ mrno, labId, publicBaseUrl }) {
  const base = String(
    publicBaseUrl ||
      process.env.PUBLIC_BASE_URL ||
      process.env.NEXT_PUBLIC_APP_URL ||
...
function resolveTrendReportDocumentUrl({ mrno, options = {}, context = {} } = {}) {
  if (!isNewTrendsReportEnabled()) {
    return getTrendReportUrl(mrno);
  }

  return (
    buildExecutiveTrendReportDocumentUrl({
      mrno,
      labId: options?.labId || context?.lab_id || null,
      publicBaseUrl: options?.publicBaseUrl || null
    }) || getTrendReportUrl(mrno)
  );
}
...
function buildCustomMoreServiceLinkMap({ options = {}, botFlowConfig = {} } = {}) {
  const map = new Map();
  const templates = options?.templates && typeof options.templates === "object" ? options.templates : {};
  const menuRows = Array.isArray(templates?.whatsapp_menus?.more_services?.rows)
    ? templates.whatsapp_menus.more_services.rows
    : [];
  const botFlowLinks = Array.isArray(botFlowConfig?.links)
    ? botFlowConfig.links
    : Array.isArray(templates?.bot_flow?.links)
      ? templates.bot_flow.links
      : [];
  const allRows = [...botFlowLinks, ...menuRows];

  for (const row of allRows) {
    if (!row || typeof row !== "object") continue;
    const url = String(row.url || row.link || "").trim();
    if (!/^https?:\/\//i.test(url)) continue;
    const intent = normalizeIntentToken(row.intent || row.id || row.key);
    if (!intent) continue;
    map.set(intent, {
      url,
      descriptor: String(row.descriptor || row.description || row.title || "Link").trim()
    });
  }
  return map;
}

function hasConfiguredCustomIntent({ intent = "", options = {}, botFlowConfig = {} } = {}) {
  ...
  const botFlowLinks = Array.isArray(botFlowConfig?.links)
    ...
    : Array.isArray(templates?.bot_flow?.links)
      ...
      : [];
  const allRows = [...botFlowLinks, ...menuRows];
  return allRows.some((row) => normalizeIntentToken(row?.intent || row?.id || row?.key) === key);
}

function handleMoreServicesInput({ input, context, phone, botFlowConfig, options = {} }) {
  ...
  if (input === "SEND_LOCATION") {
    return {
      replyType: "SEND_LOCATION_AND_BRANCHES_MENU",
  ...
  if (input === "SMART_REPORT_DOWNLOAD") {
    if (!isSmartReportEnabled({ options, botFlowConfig })) {
      return {
        replyType: "TEXT",
        replyText: "Smart Report is currently not enabled for this lab.",
        newState: "MORE_SERVICES",
        context
      };
    }
    ...
    return buildTrendReportResponse({
      ...
      botFlowConfig,
      options,
      forceSmartReport: true
    });
  }

  // Handle template-driven custom menu rows after all built-in intents
  // so standard options (timings/location/feedback/etc.) are never shadowed.
  const customLinks = buildCustomMoreServiceLinkMap({ options, botFlowConfig });
  const customLink = customLinks.get(normalizeIntentToken(input));
  if (customLink?.url) {
    return {
      replyType: "CUSTOM_LINK_TEXT",
      replyText: `${customLink.descriptor}\n${customLink.url}`,
      newState: "MORE_SERVICES",
      context
    };
  }
  if (hasConfiguredCustomIntent({ intent: input, options, botFlowConfig })) {
    return {
      replyType: "TEXT",
      replyText: "This option is temporarily unavailable right now. Please try again later.",
      newState: "MORE_SERVICES",
      context
    };
  }
...
function normalizeButtonSelection(rawInput) {
  ...
  if (!trimmed) return "";
  if (trimmed === "__LOCATION_PIN__" || trimmed === "__MEDIA__") return trimmed;
  ...
  const directMap = {
    ...
    SMART_REPORT_DOWNLOAD: "SMART_REPORT_DOWNLOAD",
    DOWNLOAD_SMART_REPORT: "SMART_REPORT_DOWNLOAD",
    "DOWNLOAD SMART REPORT": "SMART_REPORT_DOWNLOAD",
    SMART_REPORT: "SMART_REPORT_DOWNLOAD",
    "SMART REPORT": "SMART_REPORT_DOWNLOAD",
    "SMART TREND REPORT": "SMART_REPORT_DOWNLOAD",
    ...
    "REPORT LIST": "REPORT_PREVIOUS_LIST",
    "REPORTS LIST": "REPORT_PREVIOUS_LIST",
    ...
    SHARE_LOCATION_PIN: "SHARE_LOCATION_PIN",
    "MAIN LOCATION": "SHARE_LOCATION_PIN",
    "MAIN BRANCH PIN": "SHARE_LOCATION_PIN",
    "MAIN BRANCH LOCATION": "SHARE_LOCATION_PIN",
    SHARE_BRANCH_LOCATIONS: "SHARE_BRANCH_LOCATIONS",
    "OTHER BRANCHES": "SHARE_BRANCH_LOCATIONS",
    "OTHER BRANCHES PIN": "SHARE_BRANCH_LOCATIONS",
    "BRANCH LOCATIONS": "SHARE_BRANCH_LOCATIONS",
    SEND_LOCATION: "SEND_LOCATION",
    SEND_LAB_LOCATION: "SEND_LOCATION",
    SHARE_LAB_LOCATION: "SEND_LOCATION",
    SHARE_LOCATION: "SEND_LOCATION",
    "SEND LOCATION": "SEND_LOCATION",
    "SEND LAB LOCATION": "SEND_LOCATION",
    "LAB LOCATION": "SEND_LOCATION",
    LAB_LOCATION: "SEND_LOCATION",
    LOCATION: "SEND_LOCATION",
    LAB_TIMINGS: "LAB_TIMINGS",
    "LAB TIMINGS": "LAB_TIMINGS",
    "LAB TIMING": "LAB_TIMINGS",
    LAB_TIME: "LAB_TIMINGS",
    LAB_HOURS: "LAB_TIMINGS",
    WORKING_HOURS: "LAB_TIMINGS",
    EXPLORE_PACKAGES: "EXPLORE_PACKAGES",
    EXPLORE_HEALTH_PACKAGES: "EXPLORE_PACKAGES",
    HEALTH_PACKAGES: "EXPLORE_PACKAGES",
    PACKAGES: "EXPLORE_PACKAGES",
    "EXPLORE PACKAGES": "EXPLORE_PACKAGES",
  ...
  // Template row ids can vary across labs (e.g. EXPLORE_HEALTH_PACKAGES,
  // SEND_LAB_LOCATION, LAB_WORKING_HOURS). Keep mapping resilient.
  if (compact.includes("PACKAGE")) return "EXPLORE_PACKAGES";
  if (compact.includes("TIMING") || compact.includes("HOUR")) return "LAB_TIMINGS";
  if (compact.includes("OTHER") && compact.includes("BRANCH")) return "SHARE_BRANCH_LOCATIONS";
  if (compact.includes("BRANCH") && compact.includes("LOCATION")) return "SHARE_BRANCH_LOCATIONS";
  if (compact.includes("MAIN") && compact.includes("LOCATION")) return "SHARE_LOCATION_PIN";
  if (
    compact.includes("LOCATION") ||
    (compact.includes("LAB") && compact.includes("ADDRESS"))
  ) {
    return "SEND_LOCATION";
  }
  if (compact.includes("TREND") && compact.includes("REPORT")) return "DOWNLOAD_TREND_REPORTS";
  if (compact.includes("FEEDBACK") || compact.includes("RATE")) return "FEEDBACK";
  if (compact.includes("EXECUTIVE") || compact.includes("AGENT")) return "TALK_EXECUTIVE";
...
function buildReportMenuRows(reports = []) {
  return (reports || []).slice(0, 5).map((r) => ({
    ...r,
    display_title: `Req No: ${String(r.reqno || r.reqid || "").trim()} • ${formatReportDate(r.reqdt)}`
  }));
}
...
function extractSafeFirstName(value) {
...
function resolveReqidFromSelectionText(rawInput, context = {}) {
  const inputText = normalizeLooseText(rawInput);
  if (!inputText) return null;

  const options = Object.values(context.report_options || {}).filter((row) => row?.reqid);
  if (options.length === 0) return null;

  const extractedReqno = extractReqnoFromSelectionTitle(rawInput);
  if (extractedReqno) {
    const byReqno = options.find((row) => String(row.reqno || "").replace(/\D/g, "") === extractedReqno);
    if (byReqno?.reqid) return String(byReqno.reqid);
  }

  const byName = options.filter((row) => {
    const patientName = normalizeLooseText(row?.patient_name || "");
    return patientName && (patientName === inputText || patientName.includes(inputText) || inputText.includes(patientName));
  });
  if (byName.length === 1) {
    return String(byName[0].reqid);
  }

  const byReqidSuffix = options.find((row) => {
    const reqid = String(row.reqid || "").toUpperCase();
    return reqid && inputText.includes(reqid.slice(-6));
  });
  if (byReqidSuffix?.reqid) return String(byReqidSuffix.reqid);
...
// User, 2026-09-13: "this message should check status of that report and
// send it to the patient if its from the registered number" -- a report
// request that already names a specific reqno ("...for requisition
// 20260619078") shouldn't have to go through the generic Latest/Previous
// menu at all. Matches both the current "R" + 12-digit format
// (R202609120010) and the pre-cutover legacy numeric format (11 digits,
// e.g. 20260619078, per [[shivam-decommission-timeline]]) -- deliberately
// NOT 10 digits, so this never mistakes a pasted mobile number for a reqno.
function extractReqnoFromText(text) {
  const raw = String(text || "");
  const modern = raw.match(/\bR\d{12}\b/i);
  if (modern) return modern[0].toUpperCase();
  const legacy = raw.match(/\b\d{11}\b/);
  if (legacy) return legacy[0];
  return null;
}

// Ownership check is the whole point here -- report-status already carries
// the requisition's own registered phone (patient_phone/phoneno, see
// labit-core's dispatch_status_legacy_shape), so this compares it against
// the WhatsApp sender's own number and refuses (returns null, caller falls
// back to the normal menu) on ANY mismatch or lookup failure. Never sends
// a report to a number it isn't registered to, never assumes -- same
// posture as every other ownership check in this codebase.
async function buildReportByReqnoResponse({ reqno, phone, context }) {
  try {
    const cleanPhone = normalizePhone(phone);
    const senderDigits = String(cleanPhone || "").replace(/\D/g, "").slice(-10);
    if (!senderDigits) return null;

    const status = await getReportStatus(reqno);
    const statusPhone = String(
      status?.patient_phone || status?.phoneno || status?.PHONENO || ""
    ).replace(/\D/g, "").slice(-10);
    if (!statusPhone || statusPhone !== senderDigits) return null;

    const reqid = String(status?.reqid || "").trim();
    if (!reqid) return null;
    const resolvedReqno = String(status?.reqno || reqno).trim();
    const patientName = String(status?.patient_name || "").trim() || null;
    const mrno = String(status?.mrno || "").trim() || null;
    ...
    return {
      ...
      documentUrl: getReportUrl(reqid, { reqno: resolvedReqno }),
      filename: `SDRC_Report_${resolvedReqno}_${firstName}.pdf`,
      reportStatusReqno: resolvedReqno,
      fallbackRequestedInput: `Report PDF not available for requisition ${resolvedReqno}`,
      sendReportActionsMenu: true,
      newState: "REPORT_POST_DOWNLOAD_MENU",
      context: {
        ...context,
        selected_report_reqid: reqid,
        selected_report_reqno: resolvedReqno,
        ...(patientName ? { selected_report_patient_name: patientName } : {}),
        ...(mrno ? { selected_report_mrno: mrno } : {})
      }
    };
  } catch (err) {
    console.warn("Report-by-reqno lookup failed:", err?.message || String(err));
    return null;
  }
}
...
async function buildLatestReportResponse({ phone, context, botFlowConfig }) {
  try {
    ...
    try {
      ...
      console.warn("Latest report context enrichment lookup failed:", lookupErr?.message || String(lookupErr));
    }

    const reqid = String(latest?.reqid || "").trim() || null;
    const reqno = String(latest?.reqno || reqid || "").trim() || null;
    ...
    const firstName = (patientName || "Patient")
      ...
      .split(/\s+/)[0]
      .toUpperCase() || "PATIENT";
    const filename = reqno
      ? `SDRC_Report_${reqno}_${firstName}.pdf`
      : `SDRC_Latest_Report_${cleanPhone}.pdf`;
    ...
    return {
      ...
      context: {
        ...context,
        latest_report_phone: cleanPhone,
        ...(reqid ? { selected_report_reqid: reqid } : {}),
        ...(reqno ? { selected_report_reqno: reqno } : {}),
        ...(patientName ? { selected_report_patient_name: patientName } : {}),
        ...(mrno ? { selected_report_mrno: mrno } : {})
      }
    };
  ...
  try {
    ...
    if (latest?.reqid) {
      ...
      return {
        replyType: "SEND_DOCUMENT",
        documentUrl: getReportUrl(reqid, { reqno }),
...
async function buildTrendReportResponse({ phone, context, botFlowConfig, options = {}, forceSmartReport = false }) {
  try {
    const cleanPhone = normalizePhone(phone);
    const reports = await lookupReports(cleanPhone);
    console.log("Reports returned:", reports);
    ...
    if (latestReport) {
      ...
      const patientName = String(
        latestReport.patient_name || latestReport.patientnm || context?.selected_report_patient_name || ""
      ).trim();
      const firstName = extractSafeFirstName(patientName);
      const filenameStem = `${firstName || "Patient"}_${mrno || reqNo || "NA"}`;

      const smartUrl = forceSmartReport
        ? buildSmartReportDocumentUrl({
          mrno,
          labId: options?.labId || context?.lab_id || null,
          publicBaseUrl: options?.publicBaseUrl || null
        })
        : null;
      const trendUrl = forceSmartReport
        ? smartUrl
        : resolveTrendReportDocumentUrl({
          mrno,
          options,
          context
        });
      ...
      return {
        replyType: "SEND_DOCUMENT",
        documentUrl: trendUrl || getTrendReportUrl(mrno),
        filename: `${filenameStem}_${smartUrl ? "Smart_Trend_Report" : "Trend_Report"}_SDRC.pdf`,
        isTrendReportDelivery: true,
        reportStatusReqno: String(latestReport.reqno || "").trim() || null,
        suppressReportStatusMessage: true,
        ...
        context: {
          ...context,
          latest_report_phone: cleanPhone,
          selected_report_reqid: String(latestReport.reqid || "").trim() || null,
          selected_report_reqno: String(latestReport.reqno || "").trim() || null,
          selected_report_patient_name: patientName || null,
          selected_report_mrno: mrno || null
        }
      };
    }
  } catch (err) {
...
async function buildTrendLatestUnifiedResponse({ phone, context, botFlowConfig, options = {} }) {
  if (context?.selected_report_mrno) {
    const reqNo = context.selected_report_reqno || context.selected_report_mrno;
    const firstName = extractSafeFirstName(context.selected_report_patient_name || "");
    const filenameStem = `${firstName || "Patient"}_${context.selected_report_mrno || reqNo || "NA"}`;

    return {
      replyType: "SEND_DOCUMENT",
      documentUrl: resolveTrendReportDocumentUrl({
        mrno: context.selected_report_mrno,
        options,
        context
      }),
      filename: `${filenameStem}_Trend_Report_SDRC.pdf`,
      isTrendReportDelivery: true,
      suppressReportStatusMessage: true,
  ...
  const fallbackTrend = await buildTrendReportResponse({
    phone: context?.latest_report_phone || phone,
    context,
    botFlowConfig,
    options
  });

  if (fallbackTrend?.replyType === "SEND_DOCUMENT") {
    return {
      ...fallbackTrend,
      sendReportActionsMenu: true,
      newState: "REPORT_POST_DOWNLOAD_MENU",
...
export async function processMessage(session, userInput, phone, options = {}) {
  ...
  // 2026-09-13, live: "Hi, I'd like a printed/PDF copy of my report for
  // requisition 20260619078." reset straight to the Main Menu, discarding
  // intent detection above (which correctly matched REQUEST_REPORTS) --
  // this regex had no end-anchor, so it matched ANY message merely
  // starting with a greeting word, not just a bare "Hi"/"Menu". That
  // silently swallowed every intent (reports, booking, packages, ...)
  // whenever a real request happened to open with a greeting, which is
  // extremely common phrasing, not an edge case. Now only resets for the
  // greeting word itself (plus trailing punctuation/whitespace) with
  // nothing else in the message.
  if (/^(hi+|hai|hello+|hey+|menu)[\s.,!?]*$/i.test(rawInput)) {
    return {
      replyType: "MAIN_MENU",
      newState: "START",
      context: {}
  ...
  if (input === "REQUEST_REPORTS") {
    const reqnoInText = extractReqnoFromText(rawInput);
    if (reqnoInText) {
      const byReqno = await buildReportByReqnoResponse({ reqno: reqnoInText, phone, context });
      if (byReqno) return byReqno;
    }
    return {
  ...
  if (input === "DOWNLOAD_TREND_REPORTS") {
    return buildTrendLatestUnifiedResponse({
      phone,
      context,
      botFlowConfig,
      options
  ...
  // Allow branch-link selections even if state drifted from LOCATION_BRANCHES.
  ...
  {
    const globalMoreServicesResult = handleMoreServicesInput({
      input,
      context,
      phone,
      botFlowConfig,
    ...
    if (globalMoreServicesResult) {
      return globalMoreServicesResult;
    }
  }
  // ---------------------------------------------------
  // STATE MACHINE
  ...
  switch (state) {
    ...
    case "START": {
      ...
      // Same reqid-from-rawInput fix as the primary REPORT_SELECTION case above --
      // `input` is normalized (colons stripped, uppercased) and corrupts an
      // archive-tagged reqid ("archive:P1611609" -> "ARCHIVE P1611609").
      if (rawInput.startsWith("REPORT_")) {
        const reqid = rawInput.replace("REPORT_", "").trim();
        ...
        return {
          replyType: "SEND_DOCUMENT",
          documentUrl: getReportUrl(reqid, {
            reqno: entry?.reqno || fallbackReqno || undefined
          }),
    ...
    case "REPORT_WAITING_INPUT": {
      if (input === "TREND_LATEST" || input === "TREND_REPORT_LATEST" || input === "DOWNLOAD_TREND_REPORTS") {
        return buildTrendLatestUnifiedResponse({
          phone,
          context,
          botFlowConfig,
      ...
      {
        // Same reqno-priority path as the global REQUEST_REPORTS handler --
        // a follow-up reply that's just a reqno ("20260619078") should
        // resolve directly too, not fall through to a human handover.
        const reqnoInText = extractReqnoFromText(rawInput);
        if (reqnoInText) {
          const byReqno = await buildReportByReqnoResponse({ reqno: reqnoInText, phone, context });
          if (byReqno) return byReqno;
    ...
    case "REPORT_SELECTION": {
      ...
      // 2026-09-01, user-caught live: reqid must come from rawInput, not the
      // normalized `input` -- normalizeButtonSelection() strips anything
      // that isn't a letter/digit/space/underscore (so "archive:P1611609"
      // loses its colon to a space) and uppercases everything, which is
      // fine for matching loosely-typed menu text but corrupts a genuine
      // WhatsApp list-row id like "REPORT_archive:P1611609" into
      // "REPORT_ARCHIVE P1611609" -- silently 500ing the actual PDF fetch
      // for every archive-sourced report (any reqid tagged with the
      // "archive:" prefix, dispatch_lookup_service.py's merge convention).
      // rawInput is WhatsApp's own echoed id, untouched.
      let reqid = "";
      if (rawInput.startsWith("REPORT_")) {
        reqid = rawInput.replace("REPORT_", "").trim();
      } else {
        reqid = resolveReqidFromSelectionText(rawInput, context) || "";
      }
    ...
    case "BOOKING_LOCATION_WAITING_PIN":
      ...
      if (options?.inboundLocation?.latitude && options?.inboundLocation?.longitude) {
        ...
        context.area = context.area || options.inboundLocation.address || options.inboundLocation.name || null;
        return {
          replyType: "CALL_QUICKBOOK",
          replyText: getFlowText(
            botFlowConfig,
    ...
    case "BOOKING_LOCATION_WAITING_TEXT":
      if (options?.inboundLocation?.latitude && options?.inboundLocation?.longitude) {
        ...
        context.location_name = options.inboundLocation.name || null;
        context.location_address = options.inboundLocation.address || null;
        context.location_text = options.inboundLocation.address || options.inboundLocation.name || null;
        context.area =
          context.area ||
          options.inboundLocation.address ||
          options.inboundLocation.name ||
          null;
    ...
    case "LOCATION_BRANCHES":
      ...
      // Keep this flexible for template-driven/custom branch row ids.
      if (input) {
        return {
          replyType: "BRANCH_LOCATION_LINK",
          branchId: input,
