...
function normalizeButtonSelection(rawInput) {
  ...
  const directMap = {
    ...
    REPORT_PREVIOUS_TRENDS: "REPORT_PREVIOUS_TRENDS",
    REPORT_PREVIOUS: "REPORT_PREVIOUS_TRENDS",
    "PREVIOUS": "REPORT_PREVIOUS_TRENDS",
    "DOWNLOAD PREVIOUS REPORTS": "REPORT_PREVIOUS_TRENDS",
    "PREVIOUS REPORTS": "REPORT_PREVIOUS_TRENDS",
    "PREVIOUS TRENDS": "REPORT_PREVIOUS_TRENDS",
...
export async function processMessage(session, userInput, phone, options = {}) {
  ...
  const detectedIntent =
    !isStructuredId && shouldApplyIntentDetection(state)
    ...
    case "BOOKING_SERVICES_MENU": {
      ...
      if (input === "BOOKING_VIEW_ACTIVE_VISIT") {
        ...
        const detailLines = [
          ...
          activeVisit?.status ? `Status: ${String(activeVisit.status).toUpperCase()}` : null
        ].filter(Boolean);
      ...
      if (input === "BOOKING_CONTACT_EXECUTIVE") {
        ...
        const hvPhoneRaw = String(activeVisit?.executive?.phone || "").trim();
        const hvPhone = hvPhoneRaw ? hvPhoneRaw.replace(/\D/g, "") : "";
        const labName = String(options?.labName || "our lab").trim();
        const reportNotifyNumberRaw = String(options?.reportNotifyNumber || "").trim();
        const reportNotifyNumber = reportNotifyNumberRaw ? reportNotifyNumberRaw.replace(/\D/g, "") : "";

        const lines = [
          "📞 Contact Details",
          hvName
            ? `Assigned HV Executive: ${hvName}${hvPhone ? ` (${hvPhone})` : ""}`
            : "Assigned HV Executive details are being updated.",
          reportNotifyNumber
