...
export async function processMessage(session, userInput, phone, options = {}) {
  ...
  if (rawInput.toLowerCase() === "report") {
    const result = await handleReportRequest(phone);
    ...
    case "START":
      ...
      if (input === "REQUEST_REPORTS") {
        ...
        try {
          ...
          if (result.type === "text" && result.message.startsWith("We found")) {
            return {
              replyType: "TEXT",
              replyText: result.message,
          ...
          if (result.message === "No reports were found for this phone number.") {
            return {
              replyType: "INTERNAL_NOTIFY",
