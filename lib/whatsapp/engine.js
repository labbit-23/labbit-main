...
export async function processMessage(session, userInput, phone, options = {}) {
  console.log("ENGINE VERSION 2 LOADED");
  ...
  if (["HI","HELLO","HEY","HII","HLO","MENU"].includes(input)) {
    return {
      replyType: "MAIN_MENU",
      newState: "START",
      context: {}
  ...
  switch (state) {
        ...
        try {
          ...
          if (reports && reports.length > 0) {
            ...
            return {
              ...
              return {
                ...r,
                display_title: `${r.patient_name} – ${formatted}`
              };
    ...
    case "REPORT_SELECTION":
      if (["HI","HELLO","HEY","MENU","MAIN_MENU"].includes(input)) {
        return {
          replyType: "MAIN_MENU",
          newState: "START",
          context: {}
      ...
      if (input.startsWith("REPORT_")) {
        ...
        const option = input.replace("REPORT_", "").trim();

        const entry = context?.report_options?.[option];

        if (!entry) {
          return {
            replyType: "TEXT",
            replyText: "Please select a report from the list.",
            newState: "REPORT_SELECTION",
            context
          };
        }

        const safeName = entry.patient_name
          .replace(/[^a-z0-9]/gi,"_")
          .replace(/_+/g,"_")
          .replace(/^_|_$/g,"")
          .substring(0,60);
        ...
        const filename =
          `SDRC_Reports_${entry.reqno}_${safeName}.pdf`;

        return {
          replyType: "SEND_DOCUMENT",
          documentUrl:
            `${process.env.NEOSOFT_API_BASE_URL}/report/${entry.reqid}`,
          filename,
          newState: "START",
          context: {}
        };
      ...
      return {
        replyType: "TEXT",
        replyText: "Reply with 1, 2 or 3 to receive the report.",
        newState: "REPORT_SELECTION",
        context
      };
