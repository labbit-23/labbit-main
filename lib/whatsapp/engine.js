...
export async function processMessage(session, userInput, phone, options = {}) {
  ...
  // ------------------------------------
  const detectedIntent = detectIntent(rawInput);
...
// ---------------------------------------------------
// REPORT DOWNLOAD FROM ANYWHERE
// ---------------------------------------------------

if (input.startsWith("REPORT_")) {

  const reqid = input.replace("REPORT_", "").trim();
  const patientNameRaw = entry?.patient_name || "Patient";
  const firstName = patientNameRaw.split(" ").find(n => n.length > 3) || patientNameRaw.split(" ")[0];

  const cleanName = firstName.replace(/[^a-zA-Z]/g,"");

  const filename = `SDRC_Report_${reqno}_${cleanName}.pdf`;
  return {
