...
export async function sendTemplateMessage({
  ...
  templateParams = [],
  sender = null,
  logPayloadExtra = null,
  headerDocumentUrl = null,
  headerDocumentFilename = null,
  // WhatsApp Authentication-category templates with a "Copy Code" button
  // (Meta Business Manager: Buttons -> Copy Code Button) are REJECTED with
  // a generic, undiagnosable error ("Error in Message request" / webhook
  // status "failed", code -1, no detail) if the send only carries a body
  // parameter -- the Cloud API requires a SEPARATE button component for
  // these, distinct from every other template shape this function already
  // handles (marketing/utility templates send fine body-only). Confirmed
  // live 2026-09-12: three real sends of otp_labit (Authentication
  // category) failed -- two before this fix (body-only), one more with an
  // INCORRECT first attempt at the button (sub_type "copy_code" + a
  // "coupon_code" parameter -- that combination is not what Meta's API
  // actually expects, despite the button being CALLED "Copy Code" in
  // Business Manager's UI). Verified against Meta's own current developer
  // docs (developers.facebook.com/documentation/business-messaging/
  // whatsapp/templates/authentication-templates/authentication-templates/):
  // the correct wire shape for a Copy Code button is sub_type "url" with a
  // plain {type: "text", text: <code>} parameter -- the SAME shape as the
  // older one-tap-autofill URL button, not a distinct "copy_code" sub_type.
  // copyCodeValue is the same OTP string as templateParams[0] in practice
  // (the button parameter re-states the code the body already shows) --
  // passed separately rather than inferred from templateParams[0] so a
  // future non-OTP body-only caller never accidentally triggers this
  // branch.
  copyCodeValue = null
}) {
  ...
  const bodyParameters = (templateParams || []).map((item) => ({
    type: "text",
    text: String(item ?? "")
  }));
  ...
  const headerLink = String(headerDocumentUrl || "").trim();
  if (headerLink) {
    components.push({
      type: "header",
      parameters: [
        {
          ...
          document: {
            link: headerLink,
            ...(String(headerDocumentFilename || "").trim()
              ? { filename: String(headerDocumentFilename || "").trim() }
              : {})
          }
        }
      ]
  ...
  const safeCopyCode = String(copyCodeValue || "").trim();
  if (safeCopyCode) {
    components.push({
      type: "button",
      sub_type: "url",
...
export async function sendImageMessage({
  ...
  return sendWhatsApp({
    ...
    phone,
    payload,
    logText: caption || "Menu banner image sent",
    sender
  });
}
      ...
      ...(String(bannerImageUrl || "").trim()
        ? {
            header: {
              type: "image",
              image: {
                link: String(bannerImageUrl).trim()
              }
            }
          }
        : {}),
...
export async function sendFeedbackActionMenu({
  labId,
  phone,
  variant = "default"
}) {
  const isComplaintVariant = String(variant || "").toLowerCase() === "complaint";
  const bodyText = isComplaintVariant
    ? "Would you like to connect to an executive or raise a complaint?"
        ...
        buttons: [
          {
            ...
            reply: {
              id: "FEEDBACK_HELP",
              title: "Connect Executive"
            }
          },
          {
            type: "reply",
            reply: {
              id: isComplaintVariant ? "FEEDBACK_COMPLAINT" : "FEEDBACK_DONE",
              title: isComplaintVariant ? "Raise Complaint" : "Done"
            }
          },
          {
            type: "reply",
  ...
  return sendWhatsApp({
    ...
    phone,
    payload,
    logText: "Feedback action menu sent"
  });
}
