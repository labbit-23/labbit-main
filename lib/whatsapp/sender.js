...
export async function sendTemplateMessage({
  ...
  // (Meta Business Manager: Buttons -> Copy Code Button, sub_type
  // COPY_CODE) are REJECTED with a generic, undiagnosable error
  // ("Error in Message request" / webhook status "failed", code -1, no
  // detail) if the send only carries a body parameter -- the Cloud API
  // requires a SEPARATE button component for these, distinct from every
  // other template shape this function already handles (marketing/utility
  // templates send fine body-only). Confirmed live 2026-09-12: two real
  // sends of otp_labit (an Authentication-category template) both failed
  // silently this way before this fix. copyCodeValue is the same OTP
  // string as templateParams[0] in practice (the button's "coupon_code"
  // parameter re-states the code the body already shows, per Meta's own
  // Authentication template contract) -- passed separately rather than
  // inferred from templateParams[0] so a future non-OTP body-only caller
  // never accidentally triggers this branch.
  copyCodeValue = null
}) {
