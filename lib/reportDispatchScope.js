...
function extractOrgId(row) {
  ...
  return normalizeOrgIdValue(value);
}
...
async function getCentresForScopedRole(user) {
  ...
  if (role === "logistics") {
    const [centreIds, labIds] = await Promise.all([
      getAssignedCentreIds(userId),
      getLabIds(userId),
    ]);
...
export function filterRowsByOrgScope(rows, allowedOrgIds) {
  ...
  const allowed = new Set(
    scoped
      .map((v) => normalizeOrgIdValue(v))
      .filter(Boolean)
  );
  return (Array.isArray(rows) ? rows : []).filter((row) => {
    const rowOrgId = normalizeOrgIdValue(extractOrgId(row));
    if (!rowOrgId) return false;
    return allowed.has(rowOrgId);
  });
}
...
export function reportStatusMatchesOrgScope(reportStatus, allowedOrgIds) {
  ...
  const topLevelOrgId = normalizeOrgIdValue(extractOrgId(reportStatus));
  if (topLevelOrgId && allowed.has(topLevelOrgId)) return true;

  const tests = Array.isArray(reportStatus?.tests) ? reportStatus.tests : [];
  for (const row of tests) {
    const testOrgId = normalizeOrgIdValue(extractOrgId(row));
    if (testOrgId && allowed.has(testOrgId)) return true;
  }
