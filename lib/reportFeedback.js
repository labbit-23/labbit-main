...
export async function saveReportFeedback({
  ...
  const normalizedSource =
    rawSource === "whatsapp" || rawSource === "whatsapp_bot"
      ? "whatsapp_bot"
      : rawSource === "kiosk"
        ? "kiosk"
        : rawSource === "public"
        ...
        : null;
  if (!normalizedSource) {
    return { ok: false, error: { message: "Unsupported feedback source", code: "INVALID_SOURCE" } };
  }
  ...
  const payload = {
    ...
    dedupe_key: String(dedupeKey || "").trim() || null,
    metadata: metadata && typeof metadata === "object" && !Array.isArray(metadata) ? metadata : {},
    created_at: new Date().toISOString()
  };

  const hasDedupeKey = Boolean(payload.dedupe_key);
  const query = hasDedupeKey
    ? supabase
        .from("report_feedback")
        .upsert(payload, { onConflict: "dedupe_key" })
    : supabase.from("report_feedback").insert(payload);
