...
// A parameter-shaped array being non-empty is NOT the same as having data --
// live, 2026-09-13: labit-core (mid-restart / under real load, see the
// archive:<reqno>-into-UUID error storm found the same day) returned
// `parameters: [{points: []}, ...]` -- an array of parameter OBJECTS with
// zero actual data points in every one of them. The old length-only check
// below called that "usable," so the pipeline rendered a report instead of
// falling back to the clean "not available" page this file already has for
// exactly this situation (the rapid/walk-in-MRN case) -- producing epoch
// "01 Jan 1970" dates, blank patient identity, and "No section summary
// available" instead of an honest "try again" message. Now requires at
// least one parameter to carry at least one real point/history/value entry.
function hasAnyDataPoint(arr) {
  return Array.isArray(arr) && arr.some((row) => {
    const points = row?.points || row?.history || row?.values;
...
function hasUsableTrendPayload(payload) {
  ...
  if (hasAnyDataPoint(payload?.parameters)) return true;
  if (hasAnyDataPoint(payload?.tests)) return true;
  if (hasAnyDataPoint(payload?.markers)) return true;
  if (hasAnyDataPoint(payload?.items)) return true;
  return false;
}
...
export async function fetchTrendPayloadByMrno(mrno) {
  ...
  for (const url of urls) {
    ...
    if (
      ...
      typeof json === "object" &&
      json.standardized &&
      hasAnyDataPoint(json.standardized.parameters)
    ) {
