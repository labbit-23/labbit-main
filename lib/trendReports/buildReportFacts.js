...
function classifyStatus(latest, low, high) {
...
function inferSectionAndMeta(parameter = {}) {
...
function recommendationFromParameter(param) {
  if (param.is_priority_marker && param.flag === "normal" && param.quality_flag !== "bad") {
    return `${param.name} is a newer preventive marker. Keep tracking this trend in your next follow-up.`;
  }
...
function normalizeParameter(parameter = {}, maxValues = 5, cutoffIso = null, psyntaxMode = "neutral") {
  ...
  const key = String(parameter.key || "").trim();
  const inferred = inferSectionAndMeta(parameter);
  const better = betterDirectionFor(key, latest?.psyntax) || inferred.better;
  const flag = classifyStatus(latestVal, toNumber(latest?.ref_low), toNumber(latest?.ref_high));
  ...
  return {
    ...
    ref_low: toNumber(latest?.ref_low),
    ref_high: toNumber(latest?.ref_high),
...
function buildActionableInsights({ timeline, sections, triggerInsights, riskLevel }) {
  ...
  const followupPanelFor = (p) => {
    ...
    if (/\bcrp\b/.test(name)) return "CRP";
    if (/\bige\b/.test(name)) return "IgE";
    if (section.includes("vitamin") || /\bvitamin|mineral|b12|d3|calcidiol|magnesium\b/.test(name)) return null;
    if (section.includes("inflammation") || /\besr|inflammation\b/.test(name)) return null;
  ...
  for (const [category, items] of groups.entries()) {
    ...
    if (priorityCount) summary.push(`${priorityCount} priority marker`);

    categoryCards.push({
      type: "category_risk",
      category,
      title: `${category} Risk Evaluation`,
      icon: categoryIcon(category),
      severity,
      text: summary.length ? summary.join(" · ") : "Within expected trend range",
    ...
    const catCandidates = items
      .filter((p) => isFreshForAction(p))
      .filter((p) => p.quality_flag === "bad" || p.flag === "high" || p.flag === "low")
      .sort((a, b) => {
        const score = (p) => {
          if (p.quality_flag === "bad") return 4;
          if (p.flag === "high") return 3;
...
export function buildReportFacts({
  ...
  const normalizedParams = (normalizedTrend.parameters || [])
    .map((param) => normalizeParameter(param, maxChartPoints, cutoffIso, resolvedPsyntaxMode))
    .filter(Boolean)
