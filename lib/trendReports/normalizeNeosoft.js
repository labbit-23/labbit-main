...
function sanitizeUnit(value) {
  ...
  if (lower === "null" || lower === "<null>" || lower === "nil" || lower === "na" || lower === "n/a") {
    return null;
  }
  return text;
}
...
function normalizeFromTablePayload(payload = {}, options = {}) {
  ...
  return normalizeFromNeosoftRows(rows, options);
}

function normalizeFromNeosoftRows(rows = [], options = {}) {
  ...
  for (const row of rows) {
    const psyntax = String(row?.PSYNTAX ?? row?.psyntax ?? "").trim();
    if (psyntaxAllowSet && !psyntaxAllowSet.has(psyntax)) continue;

    const component = String(row?.TESTCOMPONENT || row?.testcomponent || row?.display_name || row?.name || "").trim();
    ...
    const reqDate = parseNeosoftDate(row?.REQDT || row?.reqdt || row?.date);
    const value = toFiniteNumber(row?.RESULTVALUE ?? row?.resultvalue ?? row?.value);
    if (!reqDate || value === null) continue;

    const refLow = toFiniteNumber(row?.MINVAL ?? row?.minval ?? row?.ref_low ?? row?.low);
    const refHigh = toFiniteNumber(row?.MAXVAL ?? row?.maxval ?? row?.ref_high ?? row?.high);
    const unit = sanitizeUnit(row?.UNITS ?? row?.units ?? row?.unit);
...
export function normalizeNeosoftTrendPayload(payload = {}, options = {}) {
  if (!payload || (typeof payload !== "object" && !Array.isArray(payload))) {
    throw new Error("normalizeNeosoftTrendPayload requires a payload object or rows array");
  }

  if (Array.isArray(payload)) {
    const normalizedRows = normalizeFromNeosoftRows(payload, options);
    if (normalizedRows) return normalizedRows;
  }

  const normalizedTable = Array.isArray(payload) ? null : normalizeFromTablePayload(payload, options);
