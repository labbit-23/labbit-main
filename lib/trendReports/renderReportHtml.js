import { getCatalogDescription, getRuleIcon, getSectionIcon } from "./testCatalog.js";
...
function parameterInfoLine(param = {}) {
  const catalogDescription = getCatalogDescription({
  ...
  if (/\bferritin\b/.test(text)) return "Stored-iron marker used to assess iron reserves and deficiency risk.";
  if (/\biron\b/.test(text)) return "Serum iron reflects circulating iron available for red-cell production.";
  if (/\btransferrin|tibc|uibc/.test(text)) return "Iron-transport markers used to interpret iron status and utilization.";
  if (/\bsodium|potassium|chloride|electrolyte\b/.test(text)) return "Electrolyte marker important for hydration, nerve, and muscle balance.";
  if (/\bcalcium|phosphorus|phosphate\b/.test(text)) return "Bone-mineral marker used for metabolic and endocrine balance tracking.";
  if (/\bprotein\b/.test(text)) return "Protein marker used to assess nutritional and systemic health balance.";
  if (/\balbumin\b/.test(text)) return "Major blood protein reflecting nutrition, liver synthesis, and systemic health state.";
  if (/\bglobulin\b/.test(text)) return "Protein fraction related to immune activity and chronic inflammation patterns.";
  if (/\balp|alkaline phosphatase\b/.test(text)) return "Enzyme marker linked to liver, biliary tract, and bone metabolism.";
  if (/\brbc|packed cell volume|pcv|hematocrit|mcv|mch|mchc|rdw/.test(text)) return "Red-cell index marker used to classify anemia and oxygen-carrying capacity patterns.";
  if (/\bneutrophil|lymphocyte|monocyte|eosinophil|basophil|wbc differential/.test(text)) return "White-cell differential marker used to assess immune and inflammatory response patterns.";
  if (/\bpsa\b/.test(text)) return "Prostate marker used for screening follow-up with clinical correlation.";
  if (/\bca[\s-]?\d{2,3}|cea|afp|tumou?r marker/.test(text)) return "Tumor-marker trend used only for longitudinal clinical correlation, not standalone diagnosis.";

  if (section.includes("cardiac")) return "Cardiometabolic marker tracked over time for preventive heart and vascular risk awareness.";
  if (section.includes("diabetes")) return "Metabolic marker tracked over time for glucose and insulin health trends.";
  if (section.includes("kidney")) return "Renal marker tracked over time for kidney filtration and metabolic balance awareness.";
  if (section.includes("liver")) return "Liver-health marker tracked over time for hepatobiliary and metabolic trend review.";
  if (section.includes("vitamin")) return "Micronutrient marker tracked over time for nutritional trend awareness.";
  if (section.includes("hormonal") || section.includes("stress")) return "Hormonal marker tracked over time for endocrine and stress-biology trend review.";
  if (section.includes("inflammation")) return "Inflammation marker tracked over time for chronic risk and recovery trend awareness.";
  if (section.includes("cancer")) return "Screening marker tracked over time and interpreted only with clinical correlation.";
  return "Laboratory marker tracked over time for preventive clinical trend awareness.";
}
...
function stripSummaryLabel(text) {
  ...
  return raw
    .replace(/\s+risk\s+evaluation\b/gi, "")
    .replace(/\s+key\s+markers\b/gi, "")
    .replace(/\s+window\b/gi, "")
    .trim();
}

function fallbackIconForHint(hint) {
  const clean = stripSummaryLabel(hint);
  return (
    sectionFallbackIcon(clean) ||
    sectionFallbackIcon(clean.replace(/&/g, " and ")) ||
    sectionFallbackIcon(clean.replace(/\(.*?\)/g, "").trim()) ||
    "https://sdrc.in/assets/ads/icons/cbc.png"
  );
}

function resolveIconUrl(url, hint = "") {
  const direct = absoluteAssetUrl(url);
  if (direct) return direct;
  return fallbackIconForHint(hint);
}

function iconBadgeFromAsset(url, hint = "") {
  const resolved = resolveIconUrl(url, hint);
...
function sectionFallbackIcon(sectionName) {
  const catalogIcon = getSectionIcon(sectionName);
...
function ruleIconUrl(item = {}) {
  const catalogIcon = getRuleIcon(item?.key);
  if (catalogIcon) return catalogIcon;
  ...
  return `<div class="summary-grid">${cards
    .map(
      (card) => `<div class="summary-card">
        <h4>${iconBadgeFromAsset(card.icon, card.type || card.section || card.title || "")}${escapeHtml(card.type || card.section)}</h4>
        ...
        <div class="count-row count-row-normal"><span>Normal</span><strong>${escapeHtml(card.normal ?? 0)}</strong></div>
        ${Number.isFinite(Number(card.unknown)) && Number(card.unknown) > 0 ? `<div class="count-row"><span>No Ref Range</span><strong>${escapeHtml(card.unknown)}</strong></div>` : ""}
...
function insightsList(items = [], reportFacts = {}, options = {}) {
  const hideSeverityBadges = Boolean(options?.hideSeverityBadges);
  ...
  return `<div class="insights">${items
    .map(
      (item) => {
        ...
        const showSeverity = !(hideSeverityBadges && typeRaw !== "followup_window");
        const sevText = showSeverity
          ? (typeRaw === "followup_window" ? followupLabel : severityText(item.severity))
          : "";
        ...
        <div class="insight-top">
          ...
          ${showSeverity ? `<span class="insight-sev ${typeRaw === "followup_window" ? "followup-month-tag" : ""}">${escapeHtml(sevText)}</span>` : `<span class="insight-tag insight-tag-hidden"></span>`}
        </div>
        <h5>${iconBadgeFromAsset(item.icon, item.category || item.title || item.type || "")}${escapeHtml(item.title)}</h5>
...
function sectionPage(section) {
  ...
  const cards = (section.parameters || []).map(parameterCard);
  const firstRowCards = cards.slice(0, 2).join("\n");
  const remainingCards = cards.slice(2).join("\n");
  return `<section class="page detail-page">
    <div class="detail-section">
      ...
      <div class="param-grid param-grid-first">
        ${firstRowCards}
      </div>
      ${remainingCards ? `<div class="param-grid">${remainingCards}</div>` : ""}
    </div>
  </section>`;
}
...
export function renderReportHtml(reportFacts = {}) {
  ...
  const modeBadge = isTrendsMode ? "HEALTH SUMMARY" : "SMART REPORT";
  ...
  const summaryPage = `<section class="page summary-page">
    ...
    <header class="hero">
      ...
      <div class="hero-meta">
        <div><span>Data Analysed From</span><strong>${escapeHtml(formatDate(reportFacts.timeline?.first_registered_date))}</strong></div>
    ...
    <div class="block">
      <h2>${isTrendsMode ? "Key Highlights" : "Actionable Summary"}</h2>
      ${insightsList(actionableForDisplay, reportFacts, { hideSeverityBadges: isTrendsMode })}
    </div>
  ...
  const summaryByTypePage = `<section class="page summary-page page-break">
    ${logoUrl ? `<img class="hero-top-logo" src="${escapeHtml(logoUrl)}" alt="${escapeHtml(labName)}" />` : ""}
    <div class="block">
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .count-row-normal span,
    .count-row-normal strong {
      color: var(--ok);
      font-weight: 700;
    }
    .card-top {
    ...
    .theme-basic .block {
      border-color: #dce8ee;
      box-shadow: 0 4px 10px rgba(40, 76, 92, 0.06);
    }
    .theme-basic .section-header {
      border-color: #dbe7ec;
      box-shadow: 0 3px 8px rgba(38, 68, 82, 0.06);
    }
    .theme-basic .param-card {
      border-color: #dce8ee;
      box-shadow: 0 3px 9px rgba(41, 73, 88, 0.06);
    }
    .theme-basic .mode-badge {
      background: #f26939;
      box-shadow: none;
    }
    .theme-basic .end-card {
      border-left-color: #0f8a94;
      box-shadow: 0 4px 10px rgba(40, 76, 92, 0.08);
    }
