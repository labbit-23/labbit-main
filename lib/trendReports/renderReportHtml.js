...
function parameterCard(param) {
  ...
  return `<article class="param-card">
    <div class="param-head">
      ...
      <span class="flag ${statusClass(param.flag)}">${escapeHtml(statusLabel(param.flag))}</span>
    </div>
  ...
  return `<div class="summary-grid">${cards
    .map(
      (card) => `<div class="summary-card">
        ...
        ${Number.isFinite(Number(card.priority)) ? `<div class="count-row"><span>Priority</span><strong>${escapeHtml(card.priority)}</strong></div>` : ""}
  ...
  const parameterRows = (params = []) => {
    ...
    return `<ul class="insight-params">${params.map((p) => {
      ...
      const marker = p?.is_priority_marker && cls === "p-normal"
        ? `<em class="pm-ok">Priority Marker: On Track</em>`
        : p?.is_priority_marker
          ? `<em class="pm-watch">Priority Marker: Review</em>`
          : "";
      return `<li class="${cls}">
    ...
    .map(
      (item) => {
        ...
        const sevText = typeRaw === "followup_window"
          ? formatMonthYear(reportFacts?.recommended_followup_date)
...
function sectionPage(section) {
  ...
  return `<section class="page detail-page">
    <header class="section-header section-${escapeHtml(sectionClass)}">
      <h2>${iconBadgeFromAsset(icon)}${escapeHtml(section.name)}</h2>
    </header>
...
export function renderReportHtml(reportFacts = {}) {
  ...
  const followupMonthYear = formatMonthYear(reportFacts.recommended_followup_date);
  ...
  const summaryPage = `<section class="page summary-page">
    ...
    <header class="hero">
      <div>
        ...
        <p>${escapeHtml(patientName)} · ${escapeHtml(String(patientAge))} · ${escapeHtml(String(patientGender))}</p>
  ...
  const finalRecommendations = (() => {
    ...
    for (const item of reportFacts.actionable_insights || []) {
      ...
      if (item?.type === "category_risk" && item?.text) {
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media print {
      ...
      body { background: #fff; }
      .page { max-width: none; padding: 7mm 6.5mm; }
      .page-break { page-break-before: always; }
      * {
